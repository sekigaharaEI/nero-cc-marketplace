...
description: 从禅道产品浏览地址或产品 ID 中批量提取 story/研发需求，导出为可分析的 Markdown 文件，并保留需求详情、截图链接与关键字段。适用于需要抓取禅道研发需求详情、批量导出某产品下全部需求、按计划/指派人等条件筛选、按功能分组重排并落盘的场景。只产出字段提取和按功能分组文件；不要生成需求分析报告。
---
...
先确定导出范围，再用附带脚本批量抓取 story/研发需求详情，生成适合后续分析的 Markdown 文件。若当前会话已真实加载 `zentao` MCP，可先抽样确认列表和详情字段。

默认职责是**导出需求材料**，不是撰写需求分析报告。用户说“拉一份下来分析”“导出后分析”“梳理一下”时，在本 skill 中一律按“导出成便于分析的 Markdown，并按功能分组”处理。
...
- 用户只说“拉一份下来分析”“梳理一下”“整理一下需求”时，最终产物只能是字段提取文件和/或按功能分组文件；不能生成分析报告。
...
- 当用户指定计划筛选并表达“分析/梳理”时，优先交付 `禅道_<产品名>_<计划名>_按功能分组_<YYYY-MM-DD>.md`，不是单独的分析文档。
...
7. 如果用户请求“分析/梳理/整理”，继续生成按功能分组文件，不写需求分析报告。
...
适用场景：用户拿到字段提取 Markdown 后，希望需求**按"功能"语义聚合**而不是按 ID 倒序，方便逐个功能讨论、评审或排期。用户说“拉下来分析”“梳理一下”但没有明确要求分析报告时，也优先产出按功能分组文件。
...
本 skill 的交付物是字段提取和按功能分组文件。不要生成分析报告、统计报告、需求分析、周报总结或元数据文件。
