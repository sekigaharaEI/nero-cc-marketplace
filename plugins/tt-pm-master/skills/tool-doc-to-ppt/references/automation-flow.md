# 自动化执行流程

## 重要：笔记本 ID 隔离

**每次执行必须创建新笔记本，并在后续所有命令中通过 `--notebook {NOTEBOOK_ID}` 指定该笔记本。** 不传 `--notebook` 会操作上次活跃的笔记本，导致新旧文件混在一起。

## 执行步骤

```bash
# Step 1: 创建笔记本，从返回的 JSON 中提取 notebook_id
# 笔记本名称加时间戳，避免与已有笔记本重名
# 格式: "幻灯片 - {主文档名} (YYYY-MM-DD HHmm)"
notebooklm create "幻灯片 - {主文档名} (2025-01-15 1430)" --json
# 返回示例: {"notebook_id": "abc123", ...}
# 记录 NOTEBOOK_ID=abc123，后续所有命令都必须带上 --notebook {NOTEBOOK_ID}

# Step 2: 如有大纲文件，优先上传（作为主体结构）
if outline_file:
    notebooklm source add "{outline_file}" --notebook {NOTEBOOK_ID} --json

# Step 3: 上传所有素材文档（支持多个，作为补充素材）
for file in source_files:
    notebooklm source add "{文件路径}" --notebook {NOTEBOOK_ID} --json

# Step 4: 如有参考PDF，也上传（只能一个）
if reference_pdf_path:
    notebooklm source add "{reference_pdf_path}" --notebook {NOTEBOOK_ID} --json

# Step 5: 等待文档处理完成（轮询）
notebooklm source list --notebook {NOTEBOOK_ID} --json
# 每10秒检查，直到所有 source 的 status 变为 "ready"

# Step 6: 生成幻灯片
notebooklm generate slide-deck "{final_prompt}" --notebook {NOTEBOOK_ID} --format detailed --language zh_Hans --json

# Step 7: 等待生成完成（轮询）
notebooklm artifact list --notebook {NOTEBOOK_ID} --json
# 每15秒检查，直到 status 变为 "completed"

# Step 8: 下载到文档同目录
notebooklm download slide-deck "{输出路径}" --notebook {NOTEBOOK_ID}
```

## 固定参数

| 参数 | 值 | 说明 |
|------|-----|------|
| 幻灯片格式 | `--format detailed` | 详细版 |
| 语言 | `--language zh_Hans` | 中文简体 |
